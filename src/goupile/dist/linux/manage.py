...
def create(args):
    if not re.match(r"^[a-zA-Z0-9_\-\.]+$", args.name):
        raise ValueError(
            "Name must only contain alphanumeric, '.', '-' or '_' characters"
        )
...
def load_domains(path: str) -> list[DomainConfig]:
...
def sync_domains(path: str, domains: list[DomainConfig], template: str):
    os.makedirs(path, mode=0o755, exist_ok=True)

    for domain in domains:
        config = load_config(template)

        root_directory: str = os.path.join(ROOT_DIRECTORY, domain.name)
        ini_filename: str = os.path.join(path, domain.name) + ".ini"
...
def sync_units(path: str, domains: list[DomainConfig]):
...
def sync_services(domains_list: list[DomainConfig]):
    domains: dict[str, DomainConfig] = {domain.name: domain for domain in domains_list}
            ...
            if match is not None:
                ...
                if status.running:
                    try:
                        ...
                        since = int(
                            subprocess.check_output(["date", "--date=" + start, "+%s"])
                        )
    ...
    for domain in domains.values():
        ...
        if status is None:
            ...
            run_service_command(domain.name, "restart")
        elif domain.mtime != -1 and status.since < domain.mtime:
            run_service_command(domain.name, "restart")


def run_service_command(name: str, cmd: str):
...
def make_domain_config(port: str):
    config = configparser.ConfigParser()
    config.optionxform = str
...
if __name__ == "__main__":
    ...
    _ = sub.add_argument("name", metavar="name", type=str, help="domain name")
    _ = sub.add_argument(
        "-f",
        "--force",
        dest="force",
        action="store_true",
        help="Overwrite existing domain config",
    )
    _ = sub.add_argument(
        "-p",
        "--port",
        ...
        type=decode_port,
        required=False,
        help="HTTP port or unix socket",
    )
    _ = sub.add_argument(
        "--no_sync",
        dest="sync",
        action="store_false",
        help="Skip automatic domain sync",
    )
